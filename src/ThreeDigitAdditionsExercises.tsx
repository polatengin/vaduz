...
export const ThreeDigitAdditionsExercises: FunctionComponent = () => {
  ...
  const refresh = () => {
    const temp = [];
    for (let i = 1; i <= 12; i++) {
      temp.push({ i: i, a: Math.floor(Math.random() * 1000), b: Math.floor(Math.random() * 1000), c: 0, border: "" });
    }
    return temp;
  };
        ...
        {questions.map(item => {
          return (
            <div key={item.i} className="flex flex-col items-center">
              ...
              <p>{print(item.b)}</p>
              <p>+-----</p>
              <p><input type="number" inputMode="numeric" min={0} max={9999} maxLength={3} className={`border outline-0 w-14 text-center ${item.border}`} value={item.c} onFocus={(e) => e.target.select()} onChange={(e) => { update(item, e.target.value) }} /></p>
